...
#' @param ci.level A double between 0 and 1 for the levels of the confidence intervals that should be calculated. Default is 0.95.
...
#'
#' @author
#' Shamelessly stolen from Joseph, L. and Bélisle, P. <https://www.medicine.mcgill.ca/epidemiology/Joseph/PBelisle/Aickin-Alpha-Agreement-R.html>
#'
#' @export
#'
calc_aickin <- function(tbl,
                        ...
                        maxiter = 1000)
{
  ...
  A <- sum(tbl * d)
  p0 <- A / ssize
  rows.tot <- as.vector(tbl %*% matrix(J, ncol = 1))
  cols.tot <- as.vector(matrix(J, nrow=1) %*% tbl)
  pr <- rows.tot / ssize
  pc <- cols.tot / ssize
  ...
  if (ci.type == "aickin") {
    ...
    d2L.dpridprj <- -sum(tbl[1,])/(pr[1]^2) +
      A*(2*s*U+1)*R*R*matrix(pcdiff, ncol=1) %*% matrix(pcdiff, nrow=1) # m-1 x m-1
    d2L.dpri2 <- -rows.tot[-1]/(pr[-1]^2) -
      rows.tot[1]/(pr[1]^2) + A*(2*s*U + 1)*R*R*(pcdiff^2)
    diag(d2L.dpridprj) <- d2L.dpri2

    d2L.dpridpcj <- -A*R +
      A*(2*s*U+1)*R*R* matrix(pcdiff, ncol=1) %*% matrix(prdiff, nrow=1) # m-1 x m-1
    d2L.dpridpci <- -2*A*R + A*(2*s*U+1)*R*R*prdiff*pcdiff
    diag(d2L.dpridpcj) <- d2L.dpridpci

    d2L.dpcidpcj <- -sum(tbl[,1])/(pc[1]^2) +
      A*(2*s*U+1)*R*R*matrix(prdiff, ncol=1) %*% matrix(prdiff, nrow=1) # m-1 x m-1
    d2L.dpci2 <- -cols.tot[-1]/(pc[-1]^2) -
      cols.tot[1]/(pc[1]^2) +
      A*(2*s*U + 1)*R*R*(prdiff^2)
