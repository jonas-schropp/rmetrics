...
test_that("Lambda B works with valid input", {
  expect_equal(
    unname(calc_lambda(tbl, "row", F, 0)[1]),
    0.45,
    tolerance = 0.001
  )
})
test_that("Lambda A works with valid input", {
  expect_equal(
    unname(calc_lambda(tbl, "column", F, 0)[1]),
    0.42105263157894735,
    tolerance = 0.001
  )
})
